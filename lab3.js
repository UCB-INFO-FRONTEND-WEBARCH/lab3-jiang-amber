...
const findLongestSessionUser = (data) => {
  ...
  let max = 0;
  let person = "";
  // Some for-loop logic learned from INFO206
  for (let user = 0; user < data.length; user++) {
    if (data[user].avgSessionDuration > max) {
      max = data[user].avgSessionDuration;
...
const formatSessions = (data) => {
  ...
  let format = data.map((x) => makeString(x));
  return format;
};
...
const getActiveUsers = (data) => {
  ...
  let tempArray = data.filter((data) => data.totalSessions >= 5);
  names = tempArray.map((x) => x.name);
  return names;
};
...
const getTotalSessions = (data) => {
  ...
  let tempData = data.map((x) => x.totalSessions);
  let sum = 0;
  function summation(first, second) {
    return first + second;
  }
  sum = tempData.reduce(summation);
  return sum;
};
